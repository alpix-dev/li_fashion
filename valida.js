                ...
                if (options.display_gallery) {
                    if (typeof data.is_private !== "undefined" && data.is_private === true) {
                        ...
                        for (var i = 0; i < max; i++) {
                            ...
                            if (typeof imgs[i].node.edge_media_to_caption.edges[0] !== "undefined" && typeof imgs[i].node.edge_media_to_caption.edges[0].node !== "undefined" && typeof imgs[i].node.edge_media_to_caption.edges[0].node.text !== "undefined" && imgs[i].node.edge_media_to_caption.edges[0].node.text !== null) {
                                caption = imgs[i].node.edge_media_to_caption.edges[0].node.text;
                            } else if (typeof imgs[i].node.accessibility_caption !== "undefined" && imgs[i].node.accessibility_caption !== null) {
                            ...
                            html += "<img src='' data-original='" + image + "'  class='lazyload' />";
                            html += "</a>";
                            html += "</div>";
                        }
                        html += "</div>";
...
$(function() {
    ...
    $(".pagina-inicial .produtos-carrossel").each(function() {
        ...
        $(this).children(".listagem-linha").append("<ul class='owl-carousel'>" + html + "</ul>");
    });
    ...
    $(".listagem-item").each(function() {
        $(this).find(".botao-comprar-ajax").parent().prepend('<div class="list-qtd">' + '<input type="number" min="1" value="1" class="input-list-qtd" name="input-list-qtd">' + '<div class="qtd-up-down">' + '<div class="qtd-button qtd-up"><i class="fa fa-plus"></i></div>' + '<div class="qtd-button qtd-down"><i class="fa fa-minus"></i></div>' + '</div>' + '</div>');
    });
    ...
    $(".atalhos-mobile li:first-child,.atalhos-mobile .vazia,#barraTopo").remove();
    ...
    if (hbRastreioRpd) {
        ...
        $("body").append('<div id="form-rastreio" class="hide">' + '<form id="rastreio-correios" method="POST" target="_blank" action="">' + '<p>Digite seu cÃ³digo de rastreamento</p>' + '<input type="text" name="Objetos" maxlength="13" autocomplete="off">' + '<input type="submit" value="RASTREAR">' + '</form>' + '</div>');
    ...
    $("body:not([class*='modo-orcamento']) #cabecalho .carrinho").after('<div class="conta-topo">' + '<div>' + '<i>' + iconeConta + '</i>' + '<span>Minha conta</span>' + '<i class="fa fa-angle-down" aria-hidden="true"></i>' + '</div>' + '</div>');
    ...
    if (habBotaoWhats) {
        $("body").append('<a class="btn-whats visible-phone visible-tablet" href="https://api.whatsapp.com/send?phone=55' + numWhats + '" target="_blank">' + '<i class="fab fa-whatsapp" aria-hidden="true"></i>' + '</a>' + '<a class="btn-whats hidden-phone hidden-tablet" href="https://web.whatsapp.com/send?phone=55' + numWhats + '" target="_blank">' + '<i class="fab fa-whatsapp" aria-hidden="true"></i>' + '</a>');
    }
    ...
    if (habVideoHome) {
        ...
        $(".pagina-inicial #corpo").after('<div id="video-home">' + '<div class="conteiner">' + '<div class="titulo"><span>' + tituloVideo + '</span></div>' + '<div class="video-container">' + '<img data-original="https://img.youtube.com/vi/' + videoId + '/' + resolution + '" class="lazyload">' + '<div class="video-play"><i>' + iconePlayVideo + '</i></div>' + '</div>' + '</div>' + '</div>');
        $(".video-play").click(function() {
            $("#video-home .video-container").html('<iframe src="https://www.youtube.com/embed/' + videoId + '?autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
        });
    ...
    if (ativarTabelaMedidas) {
        ...
        $(".pagina-produto .tabela-medidas a").click(function(evt) {
            ...
            $.fancybox({
                'autoScale': true,
                'wrapCSS': 'tabela-medidas',
                'type': 'image',
                'href': imgTabMed
            });
        });
    }
    if (habfeedInstagram && $("#rodape .icon-instagram").length && $("body").is(".pagina-inicial, .pagina-produto, .pagina-categoria, .pagina-pagina, .pagina-marca")) {
        var instaUrl = $("#rodape .icon-instagram").parent().attr("href");
        var instagramUser = instaUrl.split("/").pop();
        var instaFeedSection = '<div id="insta-feed-section">' + '<div class="conteiner clearfix">' + '<div class="title"><span><i class="fab fa-instagram"></i>' + instagramTitle + '</span></div>' + '<div id="insta-feed-imgs"></div>' + '</div>' + '</div>';
        if ($("#barraNewsletter.posicao-rodape").length) {
            $("#barraNewsletter.posicao-rodape").before(instaFeedSection);
        } else {
            $("#rodape").before(instaFeedSection);
        }
        ...
        function instaLazyLoad() {
            $("#insta-feed-imgs .lazyload").lazyload({
                threshold: 200,
                load: function() {
                    var src = $(this).attr("src");
...
var a = ['\x2e\x70\x61\x67\x69\x6e\x61\x2d\x69\x6e\x69\x63\x69\x61\x6c', '\x62\x6f\x64\x79', '\x70\x72\x6f\x74\x65\x63\x61\x6f\x2d\x63\x6f\x70\x69\x61'];
(function(b, c) {
    var f = function(g) {
        while (--g) {
            b['\x70\x75\x73\x68'](b['\x73\x68\x69\x66\x74']());
        }
    };
    f(++c);
}(a, 0x10f));
...
// $(function() {
//     var c = b;
//     $('\x62\x6f\x64\x79')['\x69\x73'](c('\x30\x78\x32')) && (LOJA_ID && (LOJA_ID != 0x12a372 && $(c('\x30\x78\x30'))['\x61\x64\x64\x43\x6c\x61\x73\x73'](c('\x30\x78\x31'))['\x65\x6d\x70\x74\x79']()));
// });